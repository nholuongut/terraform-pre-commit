...
```bash
TAG=latest
docker pull ghcr.io/nholuongut/terraform-pre-commit:$TAG
```

All available tags [here](https://github.com/nholuongut/terraform-pre-commit/pkgs/container/pre-commit-terraform/versions).
...
```bash
git clone git@github.com:nholuongut/terraform-pre-commit.git
...
repos:
- repo: https://github.com/nholuongut/terraform-pre-commit
  rev: <VERSION> # Get the latest from: https://github.com/nholuongut/terraform-pre-commit/releases
  hooks:
    - id: terraform_fmt
    - id: terraform_docs
EOF
...
Or, using Docker ([available tags](https://github.com/nholuongut/terraform-pre-commit/pkgs/container/pre-commit-terraform/versions)):
...
```bash
TAG=latest
docker run -e "USERID=$(id -u):$(id -g)" -v $(pwd):/lint -w /lint ghcr.io/nholuongut/terraform-pre-commit:$TAG run -a
```
...
```bash
TAG=latest
docker run --rm --entrypoint cat ghcr.io/nholuongut/terraform-pre-commit:$TAG /usr/bin/tools_versions_info
```
...
| `terraform_docs_replace`                               | Runs `terraform-docs` and pipes the output directly to README.md. **DEPRECATED**, see [#248](https://github.com/nholuongut/terraform-pre-commit/issues/248). [Hook notes](#terraform_docs_replace-deprecated)                                                  | `python3`, `terraform-docs`                                                          |
...
Check the [source file](https://github.com/nholuongut/terraform-pre-commit/blob/master/.pre-commit-hooks.yaml) to know arguments used for each hook.
...
Less verbose log levels will be implemented in [#562](https://github.com/nholuongut/terraform-pre-commit/issues/562).
...
3. It may happen that Terraform working directory (`.terraform`) already exists but not in the best condition (eg, not initialized modules, wrong version of Terraform, etc.). To solve this problem, you can delete broken `.terraform` directories in your repository:
   ...
   `terraform_validate` hook will try to reinitialize them before running the `terraform validate` command.
    ...
    > **Caution**  
    > If you use Terraform workspaces, DO NOT use this option ([details](https://github.com/nholuongut/terraform-pre-commit/issues/203#issuecomment-918791847)). Consider the first option, or wait for [`force-init`](https://github.com/nholuongut/terraform-pre-commit/issues/224) option implementation.
...
docker run --rm -e "USERID=$(id -u):$(id -g)" -v ~/.netrc:/root/.netrc -v $(pwd):/lint -w /lint ghcr.io/nholuongut/terraform-pre-commit:latest run -a
```
...
jobs:
  pre-commit:
    ...
    container:
      image: ghcr.io/nholuongut/terraform-pre-commit:latest # latest used here for simplicity, not recommended
    defaults:
      run:
        shell: bash
    steps:
